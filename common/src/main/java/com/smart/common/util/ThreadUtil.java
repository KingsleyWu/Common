...
public final class ThreadUtil {
    ...
    private static ScheduledExecutorService getScheduledByTask(final Task task) {
        ScheduledExecutorService executorService = TASK_SCHEDULED.get(task);
        if (executorService == null) {
            ...
            TASK_SCHEDULED.put(task, executorService);
        }
    ...
    private static ExecutorService createPoolByTypeAndPriority(final int type, final int priority) {
        switch (type) {
            case TYPE_SINGLE:
                ThreadFactory singleThreadFactory = new UtilsThreadFactory("single", priority);
                return new ThreadPoolExecutor(1, 1,
                        0L, TimeUnit.MILLISECONDS,
                        new LinkedBlockingQueue<Runnable>(),
                        singleThreadFactory);
            case TYPE_CACHED:
                ThreadFactory cachedThreadFactory = new UtilsThreadFactory("cached", priority);
                return new ThreadPoolExecutor(0, 200,
                        60L, TimeUnit.SECONDS,
                        new LinkedBlockingQueue<Runnable>(1024), cachedThreadFactory,
            ...
            default:
                UtilsThreadFactory fixedThreadFactory = new UtilsThreadFactory("fixed(" + type + ")", priority);
    ...
    public abstract static class Task<T> implements Runnable {
        ...
        public boolean equals(Object o) {
            ...
            Task<?> task = (Task<?>) o;
            return isSchedule == task.isSchedule &&
                    state == task.state;
        }
        ...
        public int hashCode() {
            ...
            return Objects.hash(isSchedule, state);
        }
